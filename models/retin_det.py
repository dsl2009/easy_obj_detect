...
def propsal(rpn_probs, rpn_bbox, input_gt_class_ids, input_gt_boxes):
    ...
    for b in range(config.batch_size):
        ...
        deltas_tp = tf.gather(deltas[b, :], org_idx)
...
def get_train_tensor(images, input_rpn_match,input_rpn_bbox, gt_label, gt_boxs):
    cn, bn = resnet50.fpn(images)
    rpn_class_logits, rpn_probs, rpn_bbox = get_rpns(cn)



    propsal_box = propsal(rpn_probs, rpn_bbox)

    rcnn_class_logits, rcnn_class, rcnn_bbox_deltas = fpn_classifier_graph(rois, cn)

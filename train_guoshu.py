...
def train():
    ...
    gen = data_gen.get_batch(batch_size=config.batch_size,class_name='guoshu',image_size=config.image_size,max_detect=80)
...
def detect():
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, '/home/dsl/all_check/face_detect/mianhua/model.ckpt-2021')
        for ip in glob.glob('/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/mianhua/open/*.jpg'):
            print(ip)
            img = cv2.imread(ip)
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
            org, window, scale, padding, crop = resize_image(img, min_dim=config.image_size, max_dim=config.image_size)

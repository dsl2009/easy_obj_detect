...
def start():
    ...
    while True:
        dataes = dataes = {'data': {'task_id': '1ed4ce8a-fb02-499f-9284-90408c961ef8',
                       'task_dr': '/media/dsl/20d6b919-92e1-4489-b2be-a092290668e4/xair/6cf42be3-1b5d-47c7-a9ff-e5b9a676db50',
        ...
        if dataes['status'] == 200:
            if last_task_id!=dataes['data']['task_id']:
                ...
                am_dr = os.path.join(rt, dataes['data']['task_id'])
                if os.path.exists(am_dr):
                    for x in glob.glob(os.path.join(am_dr,'*.*')):
                        os.remove(x)
                else:
                    os.makedirs(am_dr)

                handler.hebing_size(am_dr,2048,dataes['data'])
                dataes['data']['handler_dr'] = rt

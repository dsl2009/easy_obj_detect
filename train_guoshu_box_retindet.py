...
from losses.total_loss import get_tree_box_loss
from dsl_data import visual
import config
from models.guo_shu_model import get_box_logits,predict
...
def train():
    ...
    train_tensors = refine_det.model(img, input_gt_rpn_labels=conf, input_gt_boxes_offset=loc)


    gen_bdd = data_gen.get_batch(batch_size=config.batch_size,class_name='guoshu',image_size=config.image_size,max_detect=100)
    ...
    with sv.managed_session() as sess:
        for step in range(100000):
            ...
            loct,conft = np_utils.get_loc_conf_new(true_box, true_label,
            ...
            if step % 10 == 0:
                print('step:' + str(step) +
                      ' ' + 'arm_class_loss:' + str(ls[0]) +
                      ' ' + 'arm_loc_loss:' + str(ls[1])+
                      ' ' + 'odm_class_loss:' + str(ls[2])+
                      ' ' + 'odm_loc_loss:' + str(ls[3])
                      )
...
def detect():
    ...
    pred_loc, pred_confs = get_box_logits(imgs, config)
    ...
    builder = tf.saved_model.builder.SavedModelBuilder('server/export_cor_aug')
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        saver.restore(sess, '/home/dsl/all_check/obj_detect/guo_focal_new_coor_aug/model.ckpt-99951')
        ...
        for ip in images_path:
            ...
            if len(bxx) > 0:
                ...
                visual.display_instances(imgss,bxx)
